...
The R$_{50%}$ corresponding to $\mu_{ds}$ can be interpolated from the median "ida curve" and turned into median spectral acceleration values for that limit state $\hat{S}_{a, ds}$, as shown in \ref{eq:SaR}.
...
The $\mu_{ln(S_{a, ds, blg}(T_{av}))}$ are finally combined in a single lognormal curve for the building class, as described in section \ref{subsubsec:single-building-spo2ida}. \\
...
\item If \textit{vuln} =1: all $\hat{S}_{a, ds}(T_1)$ for each building are converted to mean $\mu_{ln(S_{a, ds})}(T_1)$ and then to the intensity measure in common with the rest of the buildings, $\mu_{ln(S_{a, ds, blg}(T_{av}))}$, according to eq. \ref{eq:Sa(Tav)}.\\
