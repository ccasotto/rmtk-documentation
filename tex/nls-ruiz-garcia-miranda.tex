...
The median $\hat{R}_{ds}$ values have been already computed with eq. \ref{eq:R_RGM}, and $\mu_{16\%}-\hat{R}$, $\hat{\mu}-\hat{R}$ and $\mu_{84\%}-\hat{R}$ curves can be drawn.
...
For each of these ductilities the corresponding $\hat{R}$, $R_{16\%}$, and $R_{84\%}$ values are found interpolating the aforementioned curves, and converted into $\hat{S}_{a,ds}$ and $\beta_{S_{a d}}$ according to the following equations:
...
N random S$_a$ for each of the N sampled ductility limit states are computed using $\hat{S}_{a,ds}$ and $\beta_{S_{a d}}$, and their median and the dispersion are estimated. These parameters constitute the median $\hat{S}_{a,ds}$ and the total dispersion $\beta_{S_a}$ for the considered damage state. The procedure is repeated for each damage state.
...
The overall workflow of C$_R$-based procedure is summarised in this section. The option \textit{an\_type} must be set equal to 0 and the option \textit{in\_type} according to the input at disposal. The corresponding inputs should follow the requirements described in section \ref{subsubsec:InputCr}. Given these options the code proceeds with the following steps:

\begin{enumerate}
\item
\begin{enumerate}
\item If \textit{in\_type} = 0 roof displacement at limit states and idealised pushover are read from \textit{displacement\_profile.csv} and \textit{idealised\_curve.csv} respectively.
...
\item R and C$_R$ are computed for these ductility levels, using eq. \ref{eq:R_RGM} and \ref{eq:Cr_RGM} respectively.
\item $\hat{S}_{a,ds}$ and the corresponding dispersion $\beta_{\theta d}$ due to record-to-record variability, expressed in terms of drift,  are computed using eq. \ref{eq:Sa_RGM} and \ref{eq:betaSa_RGM} respectively fore each limit state. 
\item The 16th and 84th fractiles of $\mu_{ds}$ are computed making use of eq. \ref{eq:Sa_RGM} and three curves relating $\mu$ and the 16th, 50th and 84th fractiles of R are built.
\item 

\begin{enumerate}
\item If dispersion due to uncertainty in the limit state $\beta_{\theta c}$ is different from zero, different ductility limit states are sampled for each median ductility level $\mu_{ds}$. For each of these ductilities the corresponding R, R$16%$, and R$84%$ values are found interpolating the aforementioned curves, and converted to $\hat{S}_{a,ds}$ and $\beta_{S_{a d}}$ according to eq. \ref{eq:SaR} and \ref{eq:betaR}. N random S$_a$ for each of the N sampled ductility limit states are computed using $\hat{S}_{a,ds}$ and $\beta_{S_{a d}}$.

\item If dispersion due to uncertainty in the limit state is equal to 0, R$50%$, R$16%$, and R$84%$ values corresponding to each ductility limit state are found interpolating the aforementioned curves, and converted to $\hat{S}_{a,ds}$ and $\beta_{S_{a d}}$ according to eq. \ref{eq:SaR} and \ref{eq:betaR}.
\end{enumerate}
...
\item
\begin{enumerate}
\item If \textit{vuln} = 0: in a first step the median and standard deviation of all the $\hat{S}_{a,ds}$ for each building are computed (if there were many sampled $\mu_{ds}$ for each \textit{ds}), so that a single $\hat{S}_{a,ds, blg}$ and $\beta_{S_{a d, blg}}$ is found for each building. \\
...
\item 
If \textit{vuln} =1: all $\hat{S}_{a, ds}(T_1)$ for each building are converted to mean $\mu_{ln(S_{a, ds})}(T_1)$ and then to the intensity measure in common with the rest of the buildings, $\mu_{ln(S_{a, ds, blg}(T_{av}))}$, according to eq. \ref{eq:Sa(Tav)}.\\
For the intensity measure levels defined in the variable \textit{iml} a value of loss ratio $\mu_{LR, iml}$ is defined for each $\mu_{ln(S_{a, ds})}(T_1)$ computed, and combined in a single $\mu_{LR, iml, blg}$ and standard deviation $\sigma_{LR, iml, blg}$ for each building, if $\beta_{\theta c}$ is different from 0.\\
